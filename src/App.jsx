...
/* ═══════════════════════════════════════════════════════════════
   CONFIGURATION — Edit these values to customise your site
   ═══════════════════════════════════════════════════════════════ */

const CALENDLY_URL = "#"; // Replace with your Calendly link e.g. "https://calendly.com/benlewis/discovery"
const LINKEDIN_URL = "#"; // Replace with your LinkedIn URL e.g. "https://linkedin.com/in/benlewis"
const EMAIL = "ben@benlewisltd.com";

/*
  MEDIA GUIDE — How to add your images and videos:
  
  1. Drop your files into the /public/assets/ folder
  2. Reference them below as "/assets/filename.mp4" or "/assets/filename.jpg"
  3. Each slot accepts: { type: "image", src: "/assets/photo.jpg" }
                    or: { type: "video", src: "/assets/clip.mp4" }
  4. Videos autoplay muted and loop. Images display with cover fit.
  5. Leave src as null to show the gradient placeholder.

  Supported formats:
  - Images: .jpg, .jpeg, .png, .webp
  - Videos: .mp4, .webm
  
  For best performance, compress videos to under 10MB each.
  Images should be optimised for web (1080px wide max for carousel cards).
*/
...
/* Universal media container — renders image, video, or gradient placeholder */
    ...
    if (type === "video") {
      return (
        <video
          ...
          style={{ width: "100%", aspectRatio, objectFit: "cover", borderRadius, display: "block" }}
        />
      );
    }
    return (
...
function ScrollCarousel({ items, cardWidth = 220, renderCard }) {
  ...
  return (
    <div ref={trackRef} className="hero-track"
      ...
      style={{ display: "flex", gap: "16px", overflowX: "auto", cursor: drag ? "grabbing" : "grab", padding: "0 32px 20px", scrollbarWidth: "none", WebkitOverflowScrolling: "touch" }}>
      {items.map((item, i) => renderCard(item, i, cardWidth))}
    </div>
  );
}

function CarouselCard({ item, width }) {
  return (
    <div style={{ flex: `0 0 ${width}px`, userSelect: "none" }}>
      <MediaSlot
        type={item.type}
        src={item.src}
        color={item.color}
        placeholderIcon={item.type === "video" ? "play" : "image"}
      />
    ...
    <div style={{ "--fh": "'Syne','Helvetica Neue',sans-serif", "--fb": "'Inter',-apple-system,sans-serif", minHeight: "100vh", background: "#0A0A0A", color: "#F5F0EB", fontFamily: "var(--fb)", overflowX: "hidden" }}>
      ...
      <section id="hero" style={{ minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", padding: "120px 0 60px", position: "relative" }}>
        ...
        <div style={{ marginTop: "52px", animation: "fadeUp 0.9s ease 0.7s both" }}>
          <ScrollCarousel items={HERO_ITEMS} renderCard={(item, i, w) => <CarouselCard key={item.id} item={item} width={w} />} />
        </div>
      ...
      <section id="work" style={{ padding: "80px 0 100px" }}>
        <div style={{ padding: "0 32px", maxWidth: "1100px", margin: "0 auto", textAlign: "center" }}>
          <Reveal>
            <div className="sl">The Work</div>
            <h2 className="sh">Selected <span style={{ fontWeight: 400, color: "rgba(245,240,235,0.45)" }}>pieces</span></h2>
          </Reveal>
        </div>
        <Reveal>
          <ScrollCarousel cardWidth={280} items={WORK_ITEMS} renderCard={(item, i, w) => <CarouselCard key={item.id} item={item} width={w} />} />
        </Reveal>
      ...
      <section id="ugc" style={{ padding: "80px 0 100px" }}>
        <div style={{ padding: "0 32px", maxWidth: "1100px", margin: "0 auto", textAlign: "center" }}>
          <Reveal>
            <div className="sl">UGC</div>
            <h2 className="sh">Scroll-stopping <span style={{ fontWeight: 400, color: "rgba(245,240,235,0.45)" }}>UGC</span></h2>
          </Reveal>
        </div>
        <Reveal>
          <ScrollCarousel cardWidth={260} items={UGC_ITEMS} renderCard={(item, i, w) => <CarouselCard key={item.id} item={item} width={w} />} />
        </Reveal>
        <Reveal><div style={{ textAlign: "center", marginTop: "40px" }}><a href={CALENDLY_URL} className="bg">Get this for your brand</a></div></Reveal>
      </section>

      {/* S4 — EDITORIAL & PRODUCT */}
      <section className="sp" style={{ textAlign: "center" }}>
        <Reveal>
          <div className="sl">Editorial & Product</div>
          <h2 className="sh">The full <span style={{ fontWeight: 400, color: "rgba(245,240,235,0.45)" }}>content ecosystem</span></h2>
        </Reveal>
        <div className="eg">
          {EDITORIAL_ITEMS.map((item, i) => (
            <Reveal key={i} delay={i * 0.05}><GridImage item={item} /></Reveal>
          ))}
        </div>
        <Reveal><div style={{ marginTop: "48px" }}><a href={CALENDLY_URL} className="bp">Book a Discovery Call</a></div></Reveal>
      </section>

      {/* S5 — WHAT THIS REPLACES */}
      <section style={{ padding: "120px 24px", textAlign: "center", background: "linear-gradient(180deg,#0A0A0A 0%,#0e0e0e 50%,#0A0A0A 100%)" }}>
        <Reveal>
          <div style={{ maxWidth: "700px", margin: "0 auto" }}>
            ...
            <h2 style={{ fontFamily: "var(--fh)", fontSize: "clamp(26px,4.5vw,48px)", fontWeight: 700, lineHeight: 1.12, marginBottom: "32px" }}>
              Your current content production costs
              <span style={{ display: "block", color: "#F5F0EB", marginTop: "8px" }}>£19,000–£45,000 per month.</span>
            </h2>
            <p style={{ fontSize: "15px", lineHeight: 1.8, color: "rgba(245,240,235,0.45)", maxWidth: "480px", margin: "0 auto 12px", fontWeight: 300 }}>
              Photographers. Videographers. UGC creators. Content agencies. Studio hire. Model fees.
            </p>
      ...
      <section className="sp" style={{ textAlign: "center" }}>
        <Reveal>
          ...
          <h2 className="sh">How it <span style={{ fontWeight: 400, color: "rgba(245,240,235,0.45)" }}>works</span></h2>
        </Reveal>
        <Reveal delay={0.1}>
          <div className="sr">
      ...
      <section id="about" className="sp" style={{ textAlign: "center" }}>
        <Reveal>
          <div style={{ maxWidth: "600px", margin: "0 auto" }}>
            ...
            <p style={{ fontSize: "15px", lineHeight: 1.8, color: "rgba(245,240,235,0.45)", fontWeight: 300, maxWidth: "480px", margin: "0 auto" }}>
              I build AI-powered content production systems that give DTC beauty brands campaign-grade content without the traditional production overhead. My work spans editorial fashion, skincare campaigns, and cinematic brand films.
            </p>
            <p style={{ fontSize: "15px", lineHeight: 1.8, color: "rgba(245,240,235,0.45)", fontWeight: 300, maxWidth: "480px", margin: "16px auto 0" }}>
              One partner replaces an entire production team — photographers, videographers, UGC creators, stylists, and studio sessions. You get the output. I handle the rest.
            </p>
      ...
      <section style={{ padding: "120px 24px", textAlign: "center", background: "linear-gradient(180deg,#0A0A0A 0%,#0d0d0d 100%)" }}>
        <Reveal>
          <h2 style={{ fontFamily: "var(--fh)", fontSize: "clamp(28px,5vw,52px)", fontWeight: 700, lineHeight: 1.1, maxWidth: "700px", margin: "0 auto 28px" }}>
            Ready to replace your entire content production
            <span style={{ display: "block", fontWeight: 400, color: "rgba(245,240,235,0.45)", marginTop: "4px" }}>with one partner?</span>
          </h2>
          <p style={{ fontSize: "15px", color: "rgba(245,240,235,0.45)", fontWeight: 300, maxWidth: "420px", margin: "0 auto 44px", lineHeight: 1.7 }}>
            15-minute discovery call. No pitch deck. Just a conversation about your content and how to fix it.
          </p>
